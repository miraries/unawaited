...
export const parseFiles = async function (fileList: Array<string>) {
    return await Promise.all(fileList.map(parseFile));
};

export default async function traverse(globs: Array<string>): Promise<TraverseResults> {
    ...
    const unawaitedCalls = syncFunctionCalls
        .flatMap(calledFn => {
            ...
            return definition ? [{call: calledFn, definition}] : [];
        });
